...
> `checkpoint.md` sets it to `closed`, after which the file is immutable.
>
...
<What this session set out to do. Written at the start; may be revised while still open.>

## Reasoning

<Appended as the session runs: how the thinking went, including paths tried and abandoned.
This file is the only place holding full detail — everything else is a summary of it.
Writing it here as it happens is the point: reconstructing it at the end is exactly the
lossy re-summarising the whole system exists to prevent.>
