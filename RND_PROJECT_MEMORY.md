...
|----------|--------|------|
| `check.sh` | This clone's `core.hooksPath` and `user.email`, reported first · each checkpoint's `Held by:` against this clone's identity · unfilled `<<FILL: …>>` install blanks · mechanism files against their released hashes · retired vocabulary from past MAJORs · `skeleton/` vs `.template-version` (self-hosting only) · the `.gitignore` upstream block · credential patterns supplied by the profile layer · checkpoint line limits · dangling `[S-…]` / `[EXP-…]` citations · `Resolved` in registers · `Owner:` fields carrying an address · adoption notes · counters on session filenames not yet committed · session files missing a `LOG.md` row · sessions left open · `docs/` edited without `CLAIMS.md` · tag frequencies · numbers without dates · unverified experiments · stale publications · dictionary-field value distributions | **Always exits 0.** Advisory output only |
